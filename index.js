...
app.listen(PORT, async () => {
  ...
  try {
    ...
    console.log("✅ Telegram Bots started polling successfully.");
  } catch (e) {
...
async function fetchEodhd(url) {
  ...
  try {
    ...
    throw new Error("Invalid JSON response from EODHD");
  }
}
...
async function getExchangeSymbols(exchange) {
  ...
  if (!Array.isArray(data)) throw new Error(`Invalid symbols list for ${exchange}`);
  return data
    .filter(item => {
...
async function getStockData(symbol, exchangeSuffix, minPrice) {
  ...
  if (!Number.isFinite(price) || price < minPrice) throw new Error("السعر أقل من الحد الأدنى");
...
console.log("💎 AI PRO MAX Stock Scanner Bot is fully operational.");
